...
{
		acc.cali.offset.x = myParameters.set.AccelOffset[X];
		acc.cali.offset.y = myParameters.set.AccelOffset[Y];
		acc.cali.offset.z = myParameters.set.AccelOffset[Z];
		acc.cali.scale.x = myParameters.set.AccelScale[X];
		acc.cali.scale.y = myParameters.set.AccelScale[Y];
		acc.cali.scale.z = myParameters.set.AccelScale[Z];
		acc.levelCali.scale.x = myParameters.set.ImuLevel[X];
		acc.levelCali.scale.y = myParameters.set.ImuLevel[Y];
		acc.levelCali.scale.z = myParameters.set.ImuLevel[Z];
...
void AccCalibration(Vector3ForFloat accRaw)
{
    ...
    static Vector3ForFloat new_offset;
    static Vector3ForFloat new_scale;
    static Vector3ForFloat samples[6];
    ...
    if(acc.cali.step == 6 && samples_count == 501)
    {
        ...
        if(acc.cali.success)
        {
            ...
            //保存加速度校准参数
						...
						myParameters.set.AccelOffset[X] = acc.cali.offset.x;
						myParameters.set.AccelOffset[Y] = acc.cali.offset.y;
						myParameters.set.AccelOffset[Z] = acc.cali.offset.z;
						myParameters.set.AccelScale[X] = acc.cali.scale.x;
						myParameters.set.AccelScale[Y] = acc.cali.scale.y;
						myParameters.set.AccelScale[Z] = acc.cali.scale.z;
...
void ImuLevelCalibration(void)
{
    ...
    Vector3ForFloat accAverage;
    Vector3ForFloat caliTemp;
    ...
    if(count == CALIBRATING_ACC_LEVEL_CYCLES)
    {
        ...
        if(abs(Degrees(caliTemp.x)) < 10 && abs(Degrees(caliTemp.y)) < 10)
        {
            ...
            //保存IMU安装误差校准参数
						myParameters.set.ImuLevel[X] = acc.levelCali.scale.x;
						myParameters.set.ImuLevel[Y] = acc.levelCali.scale.y;
						myParameters.set.ImuLevel[Z] = acc.levelCali.scale.z;
						ParamUpdateData(1);//写入
            //更新mavlink参数
