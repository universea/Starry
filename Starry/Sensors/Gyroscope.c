...
static void GyroDetectCheck(Vector3ForFloat gyroRaw);
...
{
		gyro.cali.offset.x = myParameters.set.GyroOffset[X];
		gyro.cali.offset.y = myParameters.set.GyroOffset[Y];
		gyro.cali.offset.z = myParameters.set.GyroOffset[Z];
		gyro.cali.scale.x = myParameters.set.GyroScale[X];
		gyro.cali.scale.y = myParameters.set.GyroScale[Y];
		gyro.cali.scale.z = myParameters.set.GyroScale[Z];
...
void GyroCalibration(Vector3ForFloat gyroRaw)
{
    ...
    Vector3ForFloat gyro_cali_temp, gyro_raw_temp;
    ...
    if(count == CALIBRATING_GYRO_CYCLES)
    {
        ...
        if(gyro.cali.success)
        {
            ...
            //保存陀螺仪校准参数
						myParameters.set.GyroOffset[X] = gyro.cali.offset.x;
						myParameters.set.GyroOffset[Y] = gyro.cali.offset.y;
						myParameters.set.GyroOffset[Z] = gyro.cali.offset.z;
						myParameters.set.GyroScale[X] = gyro.cali.scale.x;
						myParameters.set.GyroScale[Y] = gyro.cali.scale.y;
						myParameters.set.GyroScale[Z] = gyro.cali.scale.z;
...
{
    static uint32_t cnt;
    static Vector3ForFloat lastGyroRaw;
