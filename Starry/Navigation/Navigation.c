...
void VelocityEstimate(void)
{
    ...
    float deltaT;
    Vector3ForFloat gpsVel;
    Vector3ForFloat input;
    static uint32_t count;
    static bool fuseFlag;
		static Vector3ForFloat velErrorIntRate = {0.0005f, 0.0005f, 0.0005f};
    ...
    if(count++ % 40 == 0)
    {
        ...
        nav.accel_bias.z  = ConstrainFloat(nav.accel_bias.z, -0.1, 0.1);
...
//		nav.velocity.x = 0.80f * nav.velocity.x + 0.18f *  input.x + 0.02f * nav.velMeasure.x;
//		nav.velocity.y = 0.80f * nav.velocity.y + 0.18f *  input.y + 0.02f * nav.velMeasure.y;
//		nav.velocity.z = 0.80f * nav.velocity.z + 0.18f *  input.z + 0.02f * nav.velMeasure.z;
    //速度估计
...
{
    static Vector3ForFloat accelLpf;
    float accelMag;
...
static void KalmanVelInit(void)
{
    ...
    kalmanVel.fuseDelay.x = 10;    //0.22s延时
    kalmanVel.fuseDelay.y = 10;    //0.22s延时
    kalmanVel.fuseDelay.z = 10;    //0.1s延时
}
...
static void KalmanPosInit(void)
{
    ...
    //状态滑动窗口，用于解决卡尔曼状态估计量与观测量之间的相位差问题
    kalmanPos.slidWindowSize = 20;
    kalmanPos.statusSlidWindow = pvPortMalloc(kalmanPos.slidWindowSize * sizeof(kalmanPos.state));
    kalmanPos.fuseDelay.x = 20;    //0.2s延时
    kalmanPos.fuseDelay.y = 20;    //0.2s延时
    kalmanPos.fuseDelay.z = 20;    //0.05s延时
}
