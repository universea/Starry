...
int32_t VL53L0X_write_multi(uint8_t address, uint8_t reg, uint8_t *pdata, int32_t count)
{
    ...
    if(HAL_I2C_Mem_Write(&hi2c2,address,reg,I2C_MEMADD_SIZE_8BIT,pdata,count,5000)==HAL_OK)
    {
      status = STATUS_OK;
    }
    else
...
int32_t VL53L0X_read_multi(uint8_t address,  uint8_t index, uint8_t  *pdata, int32_t count)
{
    ...
    if(HAL_I2C_Master_Transmit(&hi2c2,address,&index,1,5000) == HAL_OK)
    {
        status = STATUS_FAIL;
    }
    
    if(HAL_I2C_Master_Receive(&hi2c2,address+1,pdata,count,5000) == HAL_OK)
    {
        status = STATUS_OK;
    }
    return status;
