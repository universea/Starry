...
*返 回 值: 姿态内环控制量
**********************************************************************************************************/
static Vector3ForFloat AttitudeInnerControl(Vector3ForFloat gyro, float deltaT)
{
    static Vector3ForFloat rateControlOutput;
    static Vector3ForFloat attInnerTargetLpf;
...
*返 回 值: 无
**********************************************************************************************************/
void SetAttInnerCtlTarget(Vector3ForFloat target)
{
...
void AttitudeOuterControl(void)
{
    ...
    Vector3ForFloat attOuterCtlValue;
    float 	   yawRate = 0.35f;
...
*返 回 值: 无
**********************************************************************************************************/
void SetAttOuterCtlTarget(Vector3ForFloat target)
{
...
{
    static Vector3ForFloat velLpf;
    Vector3ForFloat posInnerCtlOutput;
...
{
    static Vector3ForFloat posLpf;
    Vector3ForFloat posOuterCtlValue;
...
bool PIDReadFromFlash(void)
{
    ...
    //读取姿态内环PID参数
		fc.pid[ROLL_INNER].kP = myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KP];
		fc.pid[ROLL_INNER].kI = myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KI];
		fc.pid[ROLL_INNER].kD = myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KD];
		fc.pid[PITCH_INNER].kP = myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KP];
		fc.pid[PITCH_INNER].kI = myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KI];
		fc.pid[PITCH_INNER].kD = myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KD];
		fc.pid[YAW_INNER].kP = myParameters.set.PidAngularVelocityLoop[EULER_YAW][KP];
		fc.pid[YAW_INNER].kI = myParameters.set.PidAngularVelocityLoop[EULER_YAW][KI];
		fc.pid[YAW_INNER].kD = myParameters.set.PidAngularVelocityLoop[EULER_YAW][KD];
		//外环
		fc.pid[ROLL_OUTER].kP = myParameters.set.PidAngleLoop[EULER_ROLL][KP];
		fc.pid[ROLL_OUTER].kI = myParameters.set.PidAngleLoop[EULER_ROLL][KI];
		fc.pid[ROLL_OUTER].kD = myParameters.set.PidAngleLoop[EULER_ROLL][KD];
		fc.pid[PITCH_OUTER].kP = myParameters.set.PidAngleLoop[EULER_PITCH][KP];
		fc.pid[PITCH_OUTER].kI = myParameters.set.PidAngleLoop[EULER_PITCH][KI];
		fc.pid[PITCH_OUTER].kD = myParameters.set.PidAngleLoop[EULER_PITCH][KD];
		fc.pid[YAW_OUTER].kP = myParameters.set.PidAngleLoop[EULER_YAW][KP];
		fc.pid[YAW_OUTER].kI = myParameters.set.PidAngleLoop[EULER_YAW][KI];
		fc.pid[YAW_OUTER].kD = myParameters.set.PidAngleLoop[EULER_YAW][KD];
	
    //读取位置PID参数
		fc.pid[VEL_X].kP = myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KP];
		fc.pid[VEL_X].kI = myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KI];
		fc.pid[VEL_X].kD = myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KD];
		fc.pid[VEL_Y].kP = myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KP];
		fc.pid[VEL_Y].kI = myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KI];
		fc.pid[VEL_Y].kD = myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KD];
		fc.pid[VEL_Z].kP = myParameters.set.PidPositionVelocityLoop[EULER_YAW][KP];
		fc.pid[VEL_Z].kI = myParameters.set.PidPositionVelocityLoop[EULER_YAW][KI];
		fc.pid[VEL_Z].kD = myParameters.set.PidPositionVelocityLoop[EULER_YAW][KD];
		//外环
		fc.pid[POS_X].kP = myParameters.set.PidPositionLoop[EULER_ROLL][KP];
		fc.pid[POS_X].kI = myParameters.set.PidPositionLoop[EULER_ROLL][KI];
		fc.pid[POS_X].kD = myParameters.set.PidPositionLoop[EULER_ROLL][KD];
		fc.pid[POS_Y].kP = myParameters.set.PidPositionLoop[EULER_PITCH][KP];
		fc.pid[POS_Y].kI = myParameters.set.PidPositionLoop[EULER_PITCH][KI];
		fc.pid[POS_Y].kD = myParameters.set.PidPositionLoop[EULER_PITCH][KD];
		fc.pid[POS_Z].kP = myParameters.set.PidPositionLoop[EULER_YAW][KP];
		fc.pid[POS_Z].kI = myParameters.set.PidPositionLoop[EULER_YAW][KI];
		fc.pid[POS_Z].kD = myParameters.set.PidPositionLoop[EULER_YAW][KD];
...
static void PIDWriteToFlash(void)
{
		...
		//写入姿态内环PID参数
		myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KP] = fc.pid[ROLL_INNER].kP;
		myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KI] = fc.pid[ROLL_INNER].kI;
		myParameters.set.PidAngularVelocityLoop[EULER_ROLL][KD] = fc.pid[ROLL_INNER].kD;
		myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KP] = fc.pid[PITCH_INNER].kP;
		myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KI] = fc.pid[PITCH_INNER].kI;
		myParameters.set.PidAngularVelocityLoop[EULER_PITCH][KD] = fc.pid[PITCH_INNER].kD;
		myParameters.set.PidAngularVelocityLoop[EULER_YAW][KP] = fc.pid[YAW_INNER].kP;
		myParameters.set.PidAngularVelocityLoop[EULER_YAW][KI] = fc.pid[YAW_INNER].kI;
		myParameters.set.PidAngularVelocityLoop[EULER_YAW][KD] = fc.pid[YAW_INNER].kD;
		//外环
		myParameters.set.PidAngleLoop[EULER_ROLL][KP] = fc.pid[ROLL_OUTER].kP;
		myParameters.set.PidAngleLoop[EULER_ROLL][KI] = fc.pid[ROLL_OUTER].kI;
		myParameters.set.PidAngleLoop[EULER_ROLL][KD] = fc.pid[ROLL_OUTER].kD;
		myParameters.set.PidAngleLoop[EULER_PITCH][KP] = fc.pid[PITCH_OUTER].kP;
		myParameters.set.PidAngleLoop[EULER_PITCH][KI] = fc.pid[PITCH_OUTER].kI;
		myParameters.set.PidAngleLoop[EULER_PITCH][KD] = fc.pid[PITCH_OUTER].kD;
		myParameters.set.PidAngleLoop[EULER_YAW][KP] = fc.pid[YAW_OUTER].kP;
		myParameters.set.PidAngleLoop[EULER_YAW][KI] = fc.pid[YAW_OUTER].kI;
		myParameters.set.PidAngleLoop[EULER_YAW][KD] = fc.pid[YAW_OUTER].kD;
	
    //写入位置PID参数
		myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KP] = fc.pid[VEL_X].kP;
		myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KI] = fc.pid[VEL_X].kI;
		myParameters.set.PidPositionVelocityLoop[EULER_ROLL][KD] = fc.pid[VEL_X].kD;
		myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KP] = fc.pid[VEL_Y].kP;
		myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KI] = fc.pid[VEL_Y].kI;
		myParameters.set.PidPositionVelocityLoop[EULER_PITCH][KD] = fc.pid[VEL_Y].kD;
		myParameters.set.PidPositionVelocityLoop[EULER_YAW][KP] = fc.pid[VEL_Z].kP;
		myParameters.set.PidPositionVelocityLoop[EULER_YAW][KI] = fc.pid[VEL_Z].kI;
		myParameters.set.PidPositionVelocityLoop[EULER_YAW][KD] = fc.pid[VEL_Z].kD;
		//外环
		myParameters.set.PidPositionLoop[EULER_ROLL][KP] = fc.pid[POS_X].kP;
		myParameters.set.PidPositionLoop[EULER_ROLL][KI] = fc.pid[POS_X].kI;
		myParameters.set.PidPositionLoop[EULER_ROLL][KD] = fc.pid[POS_X].kD;
		myParameters.set.PidPositionLoop[EULER_PITCH][KP] = fc.pid[POS_Y].kP;
		myParameters.set.PidPositionLoop[EULER_PITCH][KI] = fc.pid[POS_Y].kI;
		myParameters.set.PidPositionLoop[EULER_PITCH][KD] = fc.pid[POS_Y].kD;
		myParameters.set.PidPositionLoop[EULER_YAW][KP] = fc.pid[POS_Z].kP;
		myParameters.set.PidPositionLoop[EULER_YAW][KI] = fc.pid[POS_Z].kI;
		myParameters.set.PidPositionLoop[EULER_YAW][KD] = fc.pid[POS_Z].kD;
