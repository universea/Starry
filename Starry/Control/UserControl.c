 ...
 * 
 * userControl库函数:
 * 用户操控逻辑处理，目前分为手动档（自稳）、半自动档（定高）、自动档（定点）
 *
 *   修订操作      版本号       日期         修订人
 * ------------    ------    ----------    ------------
...
static Vector3ForFloat posCtlTarget;
...
static void UpdateMaxBrakeAngle(Vector3ForFloat velocity);
...
static void AutoControl(RCCOMMAND_t rcCommand, RCTARGET_t* rcTarget)
{
    static Vector3ForFloat velCtlTarget, lastVelCtlTarget;
