...
static void ubloxDecode(uint8_t data);
...
/**********************************************************************************************************
*函 数 名: usartForUbloxOpen
*功能说明: 串口初始化
*形    参: 无
*返 回 值: 无
**********************************************************************************************************/
void usartForUbloxOpen(uint32_t baudrate)
{
...
void ubloxInit(void)
{
    ...
    {
        //打开串口
				usartForUbloxOpen(gpsBaudrateGroup[i]);
        osDelay(500);
...
//        ubloxEnableMessage(UBLOX_NAV_CLASS, UBLOX_NAV_TIMEUTC, 0);
        ...
        if(recvStatus)
        {
            if(i != 0)
            {
                ...
                //重新打开串口
								usartForUbloxOpen(GPS_DEFAULT_BAUDRATE);
                osDelay(100);
...
/**********************************************************************************************************
*函 数 名: ubloxPayloadDecode
*功能说明: ublox数据负载解析
*形    参: 输入数据
*返 回 值: 无
**********************************************************************************************************/
static void ubloxPayloadDecode(ubloxStruct_RAW_t ubloxRawData)
{
...
static void ubloxDecode(uint8_t data)
{
    static ubloxStruct_RAW_t ubloxRaw;
...
*返 回 值: 无
**********************************************************************************************************/
void ubloxSendData(uint8_t *data, uint8_t length)
{
    //串口发送
...
static void ubloxEnableMessage(uint8_t class, uint8_t id, uint8_t rate)
{
    ...
    ubloxSendData(ubloxData, dataCnt);
}
