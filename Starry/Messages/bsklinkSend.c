...
void BsklinkSendGps(uint8_t* sendFlag)
{
    ...
    //数据负载填充
    payload.time  	  = ubloxGetData().time;
    payload.numSV 	  = ubloxGetData().numSV;
    payload.fixStatus = ubloxGetData().fixStatus;
    payload.hAcc 	  = ubloxGetData().hAcc * 100;
    payload.vAcc 	  = ubloxGetData().vAcc * 100;
    payload.latitude  = ubloxGetData().latitude;
    payload.longitude = ubloxGetData().longitude;
    payload.altitude  = ubloxGetData().altitude;
    payload.velN	  = ubloxGetData().velN;
    payload.velE	  = ubloxGetData().velE;
    payload.velD	  = ubloxGetData().velD;
