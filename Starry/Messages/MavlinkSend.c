...
void MavlinkSendSysStatus(uint8_t* sendFlag)
{
    ...
    status.load = 0;//Get_OSCPUusage() * 10;
...
void MavlinkSendGpsRawInt(uint8_t* sendFlag)
{
    ...
    //消息负载赋值
    gps.time_usec = ubloxGetData().time * 1000;
    gps.lat       = ubloxGetData().latitude * (double)1e7;
    gps.lon       = ubloxGetData().longitude * (double)1e7;
    gps.alt       = ubloxGetData().altitude * 1000;
    gps.eph       = ubloxGetData().hAcc * 100;
    gps.epv       = ubloxGetData().vAcc * 100;
    gps.vel       = ubloxGetData().speed;
    gps.cog       = ubloxGetData().heading * 100;
    gps.fix_type  = ubloxGetData().fixStatus;
    gps.satellites_visible = ubloxGetData().numSV;
